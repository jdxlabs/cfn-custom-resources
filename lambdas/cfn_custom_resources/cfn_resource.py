...
def get_property(event, prop, default=None):
    properties = get_properties(event)
    if prop in properties:
        return properties[prop]
    elif default is None:
        raise Exception('Can''t find property %s' % prop)
    else:
        return default


def get_property_as_int(event, prop, default=None):
    return int(get_property(event, prop, default))


def get_physical_resource_name(event):
    return '%s-%s' % (get_properties(event)['StackName'], event['LogicalResourceId'])


def get_physical_resource_id(event, resource_id):
    return '%s-%s' % (get_physical_resource_name(event), resource_id)


def get_resource_id(event):
    tmp_input_id = event['PhysicalResourceId'].split('-')
    return tmp_input_id[len(tmp_input_id) - 1]
